...
import { View,Alert, Text, StyleSheet, TouchableOpacity, Animated, ScrollView } from 'react-native';
...
const TreePlant = () => {
  ...
  const onAnimationFinish = () => {
      ...
      if (waterLevel < 100) {
        ...
        if (waterLevel >= equilibriumLevel - 10 && waterLevel <= equilibriumLevel + 10) {
          if (mosquitoAnim.current) {
            ...
            Alert.alert('Congrats',' you passed the level! You have been able to save the earth more than 48 pounds of carbon mixture for the next 1 year '); // Show pass message
          }
        }
      }
    ...
    const stopWatering = () => {
      if (waterLevel < equilibriumLevel - 10 || waterLevel > equilibriumLevel + 10) {
        Alert.alert('Ooops!','Unfortunately you have failed the level and thereby more than 48 pounds of carbon mixture will remain in the envireonment, press ok to retry this level'); // Show failure message
